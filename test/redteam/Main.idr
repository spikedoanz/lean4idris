...
soundnessTests =
  ...
  , MkTestSpec "54-placeholder-collision-real.export"
      ...
      "reject"
  -- Round 5: New attack vectors from rigorous red teaming
  , MkTestSpec "55-rec-rule-not-validated.export"
      "Recursor rule RHS type is not validated against expected type"
      "reject"
  , MkTestSpec "56-imax-param-simplification.export"
      "IMax with params may not simplify correctly (imax(u,0) vs imax(v,0))"
      "reject"
  , MkTestSpec "57-placeholder-name-collision.export"
      "User creates constant named _local.0._local to collide with placeholders"
      "reject"
  , MkTestSpec "58-whnf-fuel-exhaustion.export"
      "Deep self-application to exhaust whnf fuel (omega combinator)"
      "reject"
  , MkTestSpec "59-nested-inductive-positivity.export"
      "Nested function in inductive arg bypasses strict positivity (F -> (Bad -> False) -> Bad)"
      "reject"
  , MkTestSpec "60-proj-wrong-struct-params.export"
      "Projection extracts field with wrong structure parameters substituted"
      "reject"
  , MkTestSpec "61-lambda-body-type-escape.export"
      "Lambda body type escapes to wrong result type (Nat->Bool declared, Bool->Nat inferred)"
      "reject"
  , MkTestSpec "62-def-eq-placeholder-bypass.export"
      "User constant _x._isDefEqBody bypasses placeholder equality check"
      "reject"
  , MkTestSpec "63-subst0single-depth-exploit.export"
      "Exploit depth tracking in subst0Single for BVar confusion"
      "reject"
  , MkTestSpec "64-opaque-def-unfold.export"
      "Opaque definition unfolds when it should be opaque"
      "reject"
  , MkTestSpec "65-universe-collapse-via-max.export"
      "Max(0,u) vs Max(u,0) not properly normalized causing universe collapse"
      "reject"
  , MkTestSpec "66-eta-expand-wrong-type.export"
      "Eta expansion applied to function with wrong domain type"
      "reject"
  , MkTestSpec "67-prop-elim-to-type.export"
      "Prop recursor eliminates to Type 1 (large elimination violation)"
      "reject"
  , MkTestSpec "68-let-escape-body.export"
      "Let body escapes variable scope (uses let-bound var outside let)"
      "reject"
  , MkTestSpec "69-nat-lit-type-spoof.export"
      "Nat literal claimed to have Bool type"
      "reject"
  , MkTestSpec "70-recursive-inductive-arg.export"
      "W-type with recursive occurrence in function argument (valid)"
      "accept"
  ]
